...
Старт: **E3 Дневник**. Нижняя панель: вкладки **Дневник** | **Профиль (E6)** и центральный **FAB «+»** для скана.  
FAB → выбор фото → **E2 Результат** → дневник. При `limit_hit` → **E4 Paywall** (реклама stub / Pro stub).  
Экран **E1 Скан** остаётся как fallback-маршрут; в основном потоке скан идёт сразу через picker.
